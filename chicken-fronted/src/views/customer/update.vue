...
const submit = async () => {
  try {
    ...
    if (valid) {
      const res = await updateCustomerAPI(form)
      if (res.data.code !== 0) {
        return false
      }
...
const init = async () => {
  ...
  if (route.query.id) {
    ...
    const { data: customer } = await getCustomerByIdAPI(id.value)
    Object.assign(form, customer.data)
    console.log(form)
  } else {
    console.log('没有id')
  ...
  <el-card>
    <el-form :model="form" :rules="rules" ref="updateRef">
      ...
      <el-form-item label="OpenID" :label-width="formLabelWidth" prop="openid">
        <el-input v-model="form.openid" autocomplete="off" :disabled="true" />
      </el-form-item>
    ...
    <el-form-item class="btn_box">
      <el-button class="submit_btn" type="danger" @click="submit"
                 :disabled="!userInfoStore.userInfo?.account.includes('admin')">
        修改
      </el-button>
